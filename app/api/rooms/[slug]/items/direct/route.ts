...
export async function POST(
  ...
  if (!input.success)
    return NextResponse.json({ error: "Invalid direct item" }, { status: 400 });
  if (!rateLimiter.check(`direct:${input.data.senderId}`, 40, 60_000))
  ...
  const item = await db.$transaction(async (tx) => {
    ...
    if (!active || active.status !== "ACTIVE" || active.expiresAt <= new Date())
      return null;
    return tx.roomItem.create({
      data: {
        id: input.data.itemId,
        roomId: room.id,
        senderId: input.data.senderId,
        type: input.data.type,
        encryptedMetadata: input.data.encryptedMetadata,
        encryptionVersion: 1,

...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const item = await db.$transaction(async (tx) => { await acquireRoomLock(tx, room.id); const active = await tx.room.findUnique({ where: { id: room.id }, select: { status: true, expiresAt: true } }); if (!active || active.status !== "ACTIVE" || active.expiresAt <= new Date()) return null; return tx.roomItem.create({ data: { id: input.data.itemId, roomId: room.id, senderId: input.data.senderId, type: input.data.type, encryptedMetadata: input.data.encryptedMetadata, encryptionVersion: 1, encryptedSize: input.data.encryptedSize, availability: "DIRECT" } }); });

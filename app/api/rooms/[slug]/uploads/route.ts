...
type ReservedSession = {
  ...
  storageKey: string;
  uploadToken: string;
  action: "initialize" | "reuse" | "replace";
...
class UploadConflictError extends Error {
  constructor(public kind: "item" | "initializing") {
    super(kind);
  }
}
...
export async function reserveUploadSession(
  ...
  return db.$transaction(async (tx) => {
    ...
    const now = new Date(),
      existingItem = await tx.roomItem.findUnique({
    ...
    if (
      ...
      (existing.roomId !== room.id ||
        ...
        existing.itemType !== input.type ||
        Number(existing.encryptedSize) !== input.encryptedSize ||
        (fileFingerprint && existing.fileFingerprint !== fileFingerprint))
    )
      throw new UploadConflictError("item");
    ...
    if (
      ...
      await tx.uploadSession.update({
        ...
        data: {
          uploadTokenHash: tokenHash(reusableToken),
          encryptedMetadata: input.encryptedMetadata,
          directDelivered: existing.directDelivered || input.directDelivered,
        },
      });
      return {
        id: existing.id,
        storageKey: existing.storageKey,
        uploadToken: reusableToken,
        action: "reuse",
      };
    }
    const initializationIsFresh =
      existing?.status === "PENDING" &&
      existing.createdAt.getTime() > now.getTime() - 60_000;
    ...
    const [stored, pending, itemCount] = await Promise.all([
      tx.roomItem.aggregate({
        where: { roomId: room.id, storageKey: { not: null } },
    ...
    const quota = validateStorageQuota({
      ...
      storedBytes:
        Number(stored._sum.encryptedSize ?? 0) +
    ...
    const replacedMultipart = existing.multipartUploadId
      ? {
          storageKey: existing.storageKey,
          uploadId: existing.multipartUploadId,
        }
      : undefined;
    await tx.uploadPart.deleteMany({ where: { sessionId: existing.id } });
    await tx.uploadSession.update({
      where: { id: existing.id },
      data: {
        storageKey,
...
const defaultDependencies: UploadRouteDependencies = {
  ...
  createMultipartUpload: (storageKey) =>
    storage.createMultipartUpload(storageKey),
  markUploading: async (sessionId, uploadId) => {
    await db.uploadSession.update({
      where: { id: sessionId },
      data: { multipartUploadId: uploadId, status: "UPLOADING" },
    });
  },
  markFailed: async (sessionId) => {
  ...
  abortMultipartUpload: (storageKey, uploadId) =>
    storage.abortMultipartUpload(storageKey, uploadId),
  deleteObject: (storageKey) => storage.deleteObject(storageKey),
};

export function createUploadsPost(
  dependencies: UploadRouteDependencies = defaultDependencies,
) {
  return async function uploadsPost(
    req: NextRequest,
    { params }: { params: Promise<{ slug: string }> },
  ) {
    if (dependencies.storageKind !== "r2")
      return NextResponse.json(
        { error: "Direct storage upload is disabled" },
        { status: 404 },
      );
    ...
    const requestKey = ephemeralRequestKey([
      slug,
      input.data.senderId,
    ...
    if (!dependencies.checkRateLimit(`upload-session:${requestKey}`))
    ...
    if (room.items.some((item) => item.id === input.data.itemId))
      return NextResponse.json(
        { error: "Item already exists" },
        { status: 409 },
      );

    const storageKey = storageObjectKey(slug),
      uploadToken = ownerToken();
    let session: ReservedSession;
    try {
      session = await dependencies.reserveSession(
        room,
        input.data,
        storageKey,
        uploadToken,
      ...
      logUploadError("[UPLOAD_SESSION_RESERVATION_FAILED]", error, {
        roomId: room.id,
        storageProvider: dependencies.storageKind,
      });
      return NextResponse.json(
    ...
    if (session.action === "reuse") {
      console.info("[UPLOAD_SESSION_REUSED]", {
        roomId: room.id,
        sessionId: session.id,
        storageProvider: dependencies.storageKind,
      });
      const completedParts = await db.uploadPart.findMany({
        where: { sessionId: session.id },
        select: { partNumber: true, etag: true, size: true },
      ...
      return NextResponse.json(
        {
          ...
          partSize: input.data.partSize,
          partCount: Math.ceil(input.data.encryptedSize / input.data.partSize),
          completedParts,
          uploadedBytes: completedParts.reduce(
    ...
    if (session.action === "replace")
      console.info("[UPLOAD_SESSION_REPLACED]", {
        roomId: room.id,
        sessionId: session.id,
        storageProvider: dependencies.storageKind,
      });
    if (session.replacedMultipart) {
      try {
        await dependencies.abortMultipartUpload(
          session.replacedMultipart.storageKey,
          session.replacedMultipart.uploadId,
        );
        await dependencies.deleteObject(session.replacedMultipart.storageKey);
      } catch (error) {
        logUploadError("[R2_MULTIPART_ABORT_FAILED]", error, {
          roomId: room.id,
          sessionId: session.id,
        });
        await dependencies.markFailed(session.id).catch((cleanupError) =>
          logUploadError("[UPLOAD_SESSION_CLEANUP_FAILED]", cleanupError, {
            roomId: room.id,
            sessionId: session.id,
          }),
        );
    ...
    try {
      ...
      logUploadError("[R2_UPLOAD_INIT_FAILED]", error, {
        roomId: room.id,
        sessionId: session.id,
        storageProvider: dependencies.storageKind,
      });
    ...
    try {
      ...
      logUploadError("[UPLOAD_SESSION_UPDATE_FAILED]", error, {
        roomId: room.id,
        sessionId: session.id,
        storageProvider: dependencies.storageKind,
      });
      await dependencies
        .abortMultipartUpload(session.storageKey, uploadId)
        .catch((cleanupError) =>
          logUploadError("[R2_MULTIPART_ABORT_FAILED]", cleanupError, {
            roomId: room.id,
            sessionId: session.id,
          }),
        );

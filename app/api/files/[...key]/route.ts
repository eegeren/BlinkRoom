...
export async function GET(_: Request, { params }: { params: Promise<{ key: string[] }> }) {
  ...
  if (storage.kind === "r2") { try { return NextResponse.redirect(await storage.getPublicOrSignedUrl(key), 307); } catch { return NextResponse.json({ error: "File unavailable" }, { status: 404 }); } }

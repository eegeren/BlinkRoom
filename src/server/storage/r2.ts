...
export class R2StorageProvider implements StorageProvider {
  ...
  async getPublicOrSignedUrl(key: string) { return getSignedUrl(this.client, new GetObjectCommand({ Bucket: this.bucket, Key: key, ResponseContentType: "application/octet-stream", ResponseContentDisposition: "attachment; filename=encrypted.bin" }), { expiresIn: env.STORAGE_SIGNED_URL_TTL_SECONDS }); }

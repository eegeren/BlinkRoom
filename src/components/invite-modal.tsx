...
export function InviteModal({ url, onClose }: { url: string; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="modal invite-modal" onMouseDown={(e) => e.stopPropagation()}>
    ...
    <div className="modal-actions"><button className="button filled" onClick={copy}>{copied ? "Copied." : "Copy link"}</button>{typeof navigator !== "undefined" && "share" in navigator && <button className="button" onClick={() => navigator.share({ title: "Join my BlinkRoom", url })}><Share2 /> Share</button>}</div>
  </section></div>;
}

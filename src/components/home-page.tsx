...
export function HomePage({ maxFileSize }: { maxFileSize: number }) {
  ...
  async function createRoomFromDrop(snapshot: DropSnapshot) {
    ...
    try {
      ...
      const validationError = uploadBatchValidationError(files, maxFileSize);
      if (validationError) { showError(validationError); return; }
      markInstantDropTiming("validation_done");
      creating.current = true;
      clearPendingRoomUpload();
      preparePendingRoomCreation(files, ttlHours);
      markInstantDropTiming("shell_render_requested");
      flushSync(() => setInstantDrop({ files, error: false }));
      markInstantDropTiming("room_request_started");
      finishInstantRoomCreation(startPendingRoomCreation(), files);
    } catch (cause) {
  ...
  function finishInstantRoomCreation(promise: ReturnType<typeof startPendingRoomCreation>, files: File[]) {
    if (!promise) { setInstantDrop((current) => current ? { ...current, error: true } : current); return; }
    const attempt = ++instantAttempt.current;
    void promise.then((room) => {
      if (instantAttempt.current !== attempt) return;
      markInstantDropTiming("room_response_received");
      setPendingRoomUpload(room.slug, files);
      clearPendingRoomCreation();
      markInstantDropTiming("route_replace_started");
      router.replace(`/r/${room.slug}#${room.roomKey}`);
    }).catch(() => {
      if (instantAttempt.current !== attempt) return;
      markInstantDropTiming("room_response_received");
      setInstantDrop((current) => current ? { ...current, error: true } : current);
    });
  }
  function retryInstantRoom() {
    if (!instantDrop) return;
    setInstantDrop({ ...instantDrop, error: false });
    finishInstantRoomCreation(retryPendingRoomCreation(), instantDrop.files);
  }
  function backFromInstantRoom() {
    instantAttempt.current += 1;
    creating.current = false;
    clearPendingRoomCreation();
    clearPendingRoomUpload();
    setInstantDrop(null);
  ...
  useEffect(() => {
    ...
    const drop = (event: DragEvent) => { if (!isFileDrag(event)) return; event.preventDefault(); dragDepth.current = 0; setDragging(false); if (creating.current || validatingDrop.current || !event.dataTransfer) return; beginInstantDropTiming(); const snapshot = snapshotDrop(event.dataTransfer); if (snapshot.files.length || snapshot.items.length) void createRoomFromDrop(snapshot); };
  ...
  return (
    <main className="home">
      ...
      <SecurityProcessPlayer expanded={securityExpanded} onExpandedChange={setSecurityExpanded} />
    </main>
  );
}

...
const baseSchema = z.object({
  ...
  ROOM_DEFAULT_TTL_HOURS: z.coerce
    .number()
    .refine((v) => [1, 6, 24, 72].includes(v))
    .default(24),
  ...
  DIRECT_CONNECTION_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .min(1000)
    .max(30000)
  ...
  AUTO_DESTROY_GRACE_SECONDS: z.coerce
    .number()
    .int()
    .min(5)
    .max(3600)
    .default(30),
  PRESENCE_LEASE_SECONDS: z.coerce.number().int().min(30).max(300).default(60),
  ONE_TIME_RESERVATION_SECONDS: z.coerce
    .number()
    .int()
    .min(60)
    .max(1800)
...
function assertR2Credentials(config: StorageEnv) {
  if (
    ...
    throw new Error(
      "Invalid R2 configuration: STORAGE_PROVIDER=r2 requires R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, and R2_BUCKET_NAME",
    );
}
export function resolveStorageRuntimeConfig(source: Record<string, unknown>) {
  const config = storageSchema.parse(source);
  assertR2Credentials(config);
  return {
    ...config,
    directUpload: config.STORAGE_PROVIDER === "r2",
...
export function requireR2Config(config: StorageEnv = env) {
  ...
  if (config.STORAGE_PROVIDER !== "r2")
    throw new Error(
      "R2 configuration requested while STORAGE_PROVIDER is not r2",
    );
  ...
  return {
    ...
    bucket,
    endpoint:
      config.R2_ENDPOINT ?? `https://${accountId}.r2.cloudflarestorage.com`,
  };
}
